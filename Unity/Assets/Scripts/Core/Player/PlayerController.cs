...
public class PlayerController : MonoBehaviour
{
    ...
    /// How much time the jumping acceleration is applied while jumping is hold.
    /// </summary>
    public float MidAirJumpCount { get { return m_midAirJumpCount; } set { m_midAirJumpCount = value; } }
    /// <summary>
    ...
    [SerializeField]
    private float m_midAirJumpCount = 1;
    private float m_usedMidAirJump = 0;
    [SerializeField]
    ...
    protected virtual void Update()
    {
        ...
        if (!m_isClimbing)
        {
            ...
            {
                m_usedMidAirJump++;
                m_platformPhysics.Acceleration = Vector3.zero;
              //  m_platformPhysics.Velocity = Vector3.zero;
                m_jumpingTimer = JumpingAccTime;
                m_platformPhysics.VSpeed = JumpingSpeed;
                return;
            }
            ...
            // Moving Action
            if (GetActionState(eControllerActions.Right) && !GetActionState(eControllerActions.Down))
            {
                m_platformPhysics.AddAcceleration(Vector2.right * HorizontalMovingAcc * m_horSpeedScale);
            }
            if (GetActionState(eControllerActions.Left) && !GetActionState(eControllerActions.Down))
            {
                m_platformPhysics.AddAcceleration(-Vector2.right * HorizontalMovingAcc * m_horSpeedScale);
            }
